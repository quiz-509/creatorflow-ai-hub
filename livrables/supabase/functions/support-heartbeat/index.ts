...
async function updateClientMemory(
  ...
  const prompt = `Tu es le Support Agent de CreatorFlow Market. Synthétise ce que tu sais sur ce client pour t'en souvenir lors du prochain projet.
...
- Type de support dont il a besoin
- Points de friction récurrents anticipés
- Niveau d'autonomie et de maîtrise technique
- Ce qui a bien fonctionné dans la relation client
- Approches à éviter
  ...
  try {
    ...
    if (!isRevision && project.client_email) {
      await updateClientMemory(
        ...
        project, deliverable.summary, deliverable.support_type, feedbackContext,
      );
    }
